...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    senderAddress.SetString(request.params[8].get_str());
        if (!senderAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Berycoin address to send from");
        else
            fHasSender=true;
    }
...
UniValue createcontract(const JSONRPCRequest& request){
    ...
    uint64_t blockGasLimit = berycoinDGP.getBlockGasLimit(chainActive.Height());
    uint64_t minGasPrice = CAmount(berycoinDGP.getMinGasPrice(chainActive.Height()));
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 6)
        throw std::runtime_error(
                ...
                "3. gasPrice  (numeric or string, optional) gasPrice BERYCOIN price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
...
UniValue sendtocontract(const JSONRPCRequest& request){
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
                ...
                "5. gasPrice  (numeric or string, optional) gasPrice Berycoin price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
