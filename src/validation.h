...
#include <berycoin/berycoinDGP.h>
#include <libethereum/ChainParams.h>
#include <libethashseal/Ethash.h>
#include <libethashseal/GenesisInfo.h>
#include <script/standard.h>
#include <berycoin/storageresults.h>


extern std::unique_ptr<BerycoinState> globalState;
...
using ExtractBerycoinTX = std::pair<std::vector<BerycoinTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashBerycoinRoot,
    const dev::h256& oldHashStateRoot, const std::vector<BerycoinTransaction>& transactions);
...
class BerycoinTxConverter{

public:

    BerycoinTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionBerycoinTransactions(ExtractBerycoinTX& berycoinTx);
...
private:
    ...
    BerycoinTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<BerycoinTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
