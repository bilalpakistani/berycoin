...
#include "berycoinstate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

BerycoinState::BerycoinState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = BerycoinState::openDB(_path + "/berycoinDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

BerycoinState::BerycoinState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute BerycoinState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, BerycoinTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createBerycoinAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            berycoin::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> BerycoinState::vins() const // temp
{
...
void BerycoinState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* BerycoinState::vin(dev::Address const& _a) const
{
    return const_cast<BerycoinState*>(this)->vin(_a);
}

Vin* BerycoinState::vin(dev::Address const& _addr)
{
...
// void BerycoinState::commit(CommitBehaviour _commitBehaviour)
// {
...
void BerycoinState::kill(dev::Address _addr)
{
...
void BerycoinState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address BerycoinState::createBerycoinAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void BerycoinState::deleteAccounts(std::set<dev::Address>& addrs){
...
void BerycoinState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void BerycoinState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
