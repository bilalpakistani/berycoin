#include "berycoinDGP.h"

void BerycoinDGP::initDataEIP158(){
...
bool BerycoinDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule BerycoinDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t BerycoinDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t BerycoinDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t BerycoinDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t BerycoinDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool BerycoinDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void BerycoinDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void BerycoinDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void BerycoinDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void BerycoinDGP::createParamsInstance(){
...
dev::Address BerycoinDGP::getAddressForBlock(unsigned int blockHeight){
...
void BerycoinDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BerycoinDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BerycoinDGP::parseStorageOneUint64(uint64_t& value){
...
void BerycoinDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule BerycoinDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
