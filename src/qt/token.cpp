...
bool ToHash160(const std::string& strBerycoinAddress, std::string& strHash160)
{
    CBitcoinAddress berycoinAddress(strBerycoinAddress);
    if(berycoinAddress.IsValid()){
        CKeyID keyid;
        berycoinAddress.GetKeyID(keyid);
...
bool ToBerycoinAddress(const std::string& strHash160, std::string& strBerycoinAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress berycoinAddress;
    berycoinAddress.Set(keyid);
    if(berycoinAddress.IsValid()){
        strBerycoinAddress = berycoinAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToBerycoinAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToBerycoinAddress(tokenEvent.receiver, tokenEvent.receiver);
