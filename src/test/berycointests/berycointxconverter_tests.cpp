...
void checkResult(bool isCreation, std::vector<BerycoinTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    BerycoinTxConverter converter(transaction, NULL);
    ExtractBerycoinTX berycoinTx;
    BOOST_CHECK(converter.extractionBerycoinTransactions(berycoinTx));
    std::vector<BerycoinTransaction> result = berycoinTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(berycointxconverter_tests, TestingSetup)
