...
void initState(){
    ...
    const std::string dirBerycoin = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<BerycoinState>(new BerycoinState(dev::u256(0), BerycoinState::openDB(dirBerycoin, hashDB, dev::WithExisting::Trust), dirBerycoin + "/berycoinDB", dev::eth::BaseState::Empty));
...
dev::Address createBerycoinAddress(dev::h256 hashTx, uint32_t voutNumber){
...
BerycoinTransaction createBerycoinTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    BerycoinTransaction txEth;
    if(recipient == dev::Address()){
        txEth = BerycoinTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = BerycoinTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<BerycoinTransaction> txs){
