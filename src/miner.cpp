...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setBerycoinSchedule(berycoinDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = berycoinDGP.getBlockSize(nHeight);
    minGasPrice = berycoinDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = berycoinDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    BerycoinTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractBerycoinTX resultConverter;
    if(!convert.extractionBerycoinTransactions(resultConverter)){
    ...
    for(BerycoinTransaction berycoinTransaction : berycoinTransactions){
        txGas += berycoinTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + berycoinTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(berycoinTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, berycoinTransactions, hardBlockGasLimit);
...
void BlockAssembler::addPackageTxs(int &nPackagesSelected, int &nDescendantsUpdated, uint64_t minGasPrice)
{
    ...
    while (mi != mempool.mapTx.get<ancestor_score_or_gas_price>().end() || !mapModifiedTx.empty())
    {
        ...
        bool wasAdded=true; // BERYCOIN_INSERT_LINE
...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("berycoincoin-miner");
...
void StakeBerycoins(bool fStake, CWallet *pwallet)
{
